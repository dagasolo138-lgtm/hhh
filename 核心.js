...
import { 读取统一委托流 } from './市场/统一委托流.js';

function 读取扩展界面模型(状态) {
  初始化交易制度状态(状态);
  const 原模型 = 读取界面模型(状态);
  const 委托 = 读取玩家全部限价委托(状态);
  const 统一委托流 = 读取统一委托流(状态);
  const 可见盘口 = 统一委托流.可见盘口;
  return Object.freeze({
    ...原模型,
    市场: {
      ...原模型.市场,
      买一价: 可见盘口.买方[0]?.价格 ?? 原模型.市场.买一价,
      卖一价: 可见盘口.卖方[0]?.价格 ?? 原模型.市场.卖一价,
...
export function 创建新市场(随机种子) {
  return 创建开盘增强界面市场({ 随机种子 });
}

export function 推进一步(状态) {
  const 日志 = 推进开盘增强界面市场(状态);
...
export function 玩家限价交易(状态, 方向, 数量, 价格) {
  ...
  if (!结果.成功) return { 成功: false, 原因: 结果.原因 };
  return { 成功: true, 订单: 结果.订单, 撮合结果: 结果.撮合结果, 日志: 读取扩展界面模型(状态).日志.at(-1) };
}
...
export function 读取账户概况(状态) {
  ...
  return { 持仓市值: 账户.持仓市值, 总资产: 账户.总资产, 浮动盈亏: 账户.浮动盈亏, 总盈亏: 账户.总盈亏 };
}
...
export function 创建分析资料(状态, 最近条数 = 80) {
  ...
  return Object.freeze({
    ...
    集合竞价状态: {
      ...
      自动摘要: 状态.结构.集合竞价.自动摘要 ?? null,
    },
  });
}

export function 格式化市场时间(时间) {
  return new Date(时间).toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false });
}
