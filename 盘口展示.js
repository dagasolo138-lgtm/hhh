...
function 渲染自动委托(状态) {
  ...
  自动委托容器.innerHTML = 委托.map((订单) => {
    ...
    return `<div class="自动委托卡 ${类名}">
      <b>${转义(订单.来源)}</b>
      <span>${转义(订单.方向)} ${数量(订单.申请数量)}份</span>
      <span>限价 ${价格(订单.限价)}</span>
      <span>成交 ${数量(订单.成交数量)} / 剩余 ${数量(订单.剩余数量)}</span>
...
function 刷新() {
  const 状态 = 读取存档状态();
  if (!状态) return;
  const 指纹 = `${状态.系统?.演算序号 || 0}:${状态.结构?.统一委托流?.版本 || 0}:${状态.结构?.玩家委托?.待处理?.length || 0}:${状态.结构?.集合竞价?.待处理?.length || 0}`;
  if (指纹 === 上次指纹) return;
  上次指纹 = 指纹;
  渲染盘口(状态);
  渲染自动委托(状态);
